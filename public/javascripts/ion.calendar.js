...
var addAttendance=function(){
 ...
 $.ajax({
  type:'GET',
  url:rootUrl+'/users/todayAttendance',
...
var currentData,currentDutyHour;
    } catch(e){
...
dataType: "json",
                    ...
                    url:rootUrl+'/users/myAttendance/'+timeNowLocal.month(),
                     success: function(responseText, status, xhr){
                    if (responseText.status == 1) {currentData= responseText.msg[0]['attendance'];
                    currentDutyHour=responseText.msg[0]['dutyHours'];
                    prepareData();
                    prepareCalendar();
                    prepareAttendance();
                      ...
                      for(i = 1; i <= monthLastDay; i++) {
                            ...
                            if(currentData&&currentData.hasOwnProperty(thisMonth)){
                              ...
                              if(currentData[thisMonth][i]){
                                ...
                                if(OT){otHours+=OT;html += '<td class="ic__day ic__day_state_attendance_OT">'+i+'</td>';}

                else if(!OT||OT==0){
                                html += '<td class="ic__day ic__day_state_attendance_regular">'+i+'</td>';
                }
